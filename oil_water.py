...
class Solver:
    ...
    @staticmethod
    def count_c1_p(layer, fi, s_water, ro_water, index ):
        return (fi[index] * s_water[index] * layer.ro_water_0 * layer.c_f_water) + (s_water[index] * ro_water[index] * layer.c_r * layer.fi_0)

    @staticmethod
    def count_c2_p(layer, fi, s_water, ro_oil, index):
        return (1.0 - s_water[index]) * (fi[index] * layer.ro_oil_0 * layer.c_f_oil + layer.c_r * layer.fi_0 * ro_oil[index])
...
while solver.count_norm(delta_list_k) > solver.delta_max:
    ...
    for i in range(layer.N - 2):
        ...
        c1_p = solver.count_c1_p(layer, fi_list, s_water[1:-1], ro_water, i)
        c2_p = solver.count_c2_p(layer, fi_list, s_water[1:-1], ro_oil, i)
