...
class OilWaterSS(OilWater):
    def check_saturation_convergence(self, cell_container):
        if self.count_max_elem(
                [cell.get_cell_state_n_plus().get_s_water() - cell.get_cell_state_n().get_s_water() for cell in
                 cell_container.get_cells()]) > 12987:
            print("S_n+1 - S_n > 0.1")
            return True
        else:
            return False
    ...
    def update_saturation(self, cell_container, flow_array):
        for cell in cell_container.get_cells()[1:-1]:
            ...
            p_cap = state_n_plus.get_pressure_oil() - state_n_plus.get_pressure_water()
    ...
    def improve(self, cell_container, flow_array):
        for cell in cell_container.get_cells()[1:-1]:
            state_n_plus = cell.get_cell_state_n_plus()
            ...
            s_water_graph = cell.layer.count_s_water_graph()
            p_cap = state_n_plus.get_pressure_cap()

            s_der = cell.layer.count_s_water_graph_der(p_cap_graph, state_n.get_s_water())
            s_der = cell.layer.count_s_water_graph_der_from_p(s_water_graph, p_cap)
            s_water_new = state_n.get_s_water() + s_der * (state_n_plus.get_pressure_cap() - state_n.get_pressure_cap())
            #print("be")
    ...
    def update_pressure(cell_container, delta_list_k):
        for i in range(1, cell_container.get_len() - 1):
            ...
            cell.get_cell_state_n_plus().set_pressure_cap(cell.get_cell_state_n_plus().get_pressure_oil() - cell.get_cell_state_n_plus().get_pressure_water())
    ...
    def check_pressure_cap_convergence(self, cell_container, delta_list_k):
        ...
        for i in range(1, cell_container.get_len() - 1):
            cell = cell_container.get_cells()[i]
            p_cap_conv = (delta_list_k[i - 1][1][0] - delta_list_k[i - 1][0][
                0]) / cell.get_cell_state_n().get_pressure_cap()
    ...
    def count_b(solverSlau, cell_flow):
        ...
        # Метод Ньютона
        t_der_11 = -cell_flow.t_oil_water_der[Components.WATER.value] * (
                right_cell.get_cell_state_n().get_pressure_water() - left_cell.get_cell_state_n().get_pressure_water())

        t_der_12 = cell_flow.t_oil_water_der[Components.WATER.value] * (
                right_cell.get_cell_state_n().get_pressure_oil() - left_cell.get_cell_state_n().get_pressure_oil())

        t_der_21 = -cell_flow.t_oil_water_der[Components.OIL.value] * (
                right_cell.get_cell_state_n().get_pressure_water() - left_cell.get_cell_state_n().get_pressure_water())

        t_der_22 = cell_flow.t_oil_water_der[Components.OIL.value] * (
                right_cell.get_cell_state_n().get_pressure_oil() - left_cell.get_cell_state_n().get_pressure_oil())

        t_der = np.matrix([[t_der_11, t_der_12], [t_der_21, t_der_22]])
        ...
        if 0 <= left_cell.get_eq_index()[0] < solverSlau.e_count:
            #if left_cell.get_eq_index()[0] == solverSlau.e_count - 1:
                #t_der = np.matrix([[0.0, 0.0], [0.0, 0.0]])
            solverSlau.add_coefficient(left_cell.get_eq_index(), left_cell.get_eq_index(), t_der)
            #if 0 <= right_cell.get_eq_index()[0] < solverSlau.e_count:
            #solverSlau.add_coefficient(left_cell.get_eq_index(), right_cell.get_eq_index(), t_der)
    ...
    def count_c(solverSlau, cell_flow):
        ...
        # Метод Ньютона
        t_der_11 = -cell_flow.t_oil_water_der[Components.WATER.value] * (
                    left_cell.get_cell_state_n().get_pressure_water() - right_cell.get_cell_state_n().get_pressure_water())

        t_der_12 = cell_flow.t_oil_water_der[Components.WATER.value] * (
                left_cell.get_cell_state_n().get_pressure_oil() - right_cell.get_cell_state_n().get_pressure_oil())
        ...
        # solverSlau.set_matrix_coefficients_ss(cell_flow.get_right_cell().get_eq_index()[0],
        #                                      cell_flow.get_left_cell().get_eq_index()[0], t_der)

        if 0 <= right_cell.get_eq_index()[0] < solverSlau.e_count:
            solverSlau.add_coefficient(cell_flow.get_right_cell().get_eq_index()[0],
                                           cell_flow.get_left_cell().get_eq_index()[0], t_der)
           # solverSlau.add_coefficient(cell_flow.get_right_cell().get_eq_index()[0],
           #                                cell_flow.get_right_cell().get_eq_index()[0], t_der)
